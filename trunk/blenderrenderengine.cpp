...
BlenderRenderEngine::~BlenderRenderEngine()
{
    ...
    //blender->deleteLater(); -> No need to delete, since it should already be deleted after rendering.
    if(blender != NULL)
    {
        blender->deleteLater();
    }
...
{
    qDebug() << "BlenderRenderEngine::quit():1";
    ...
    qDebug() << "BlenderRenderEngine::quit():2";
    ...
    if(blender!=NULL)
    {
        disconnect(blender, SIGNAL(started()), this, SLOT(onEngineStarted()));
        disconnect(blender, SIGNAL(bytesWritten(qint64)), this, SLOT(onEngineBytesWritten(qint64)));
        disconnect(blender, SIGNAL(error(QProcess::ProcessError)), this, SLOT(onEngineError(QProcess::ProcessError)));
        disconnect(blender, SIGNAL(finished(int)), this, SLOT(onEngineFinished()));
        disconnect(blender, SIGNAL(aboutToClose()), this, SLOT(onEngineAboutToClose()));
        disconnect(blender, SIGNAL(readChannelFinished()), this, SLOT(onEngineReadChannelFinished()));
        disconnect(blender, SIGNAL(readyRead()), this, SLOT(onEngineReadyRead()));
        disconnect(blender, SIGNAL(destroyed()), this, SLOT(onEngineDestroyed()));
        disconnect(blender, SIGNAL(readyReadStandardOutput()), this, SLOT(onEngineReadyRead()));
        delete blender;
        blender = NULL;
    }
    qDebug() << "BlenderRenderEngine::quit():3";


    blenderError = false;
    blenderWarning = false;

    qDebug() << "BlenderRenderEngine::quit():4";
...
void BlenderRenderEngine::onEngineReadyRead()
{
    ...
    if(blender->canReadLine()) //Possible crash
    {
        QByteArray result = blender->readAll(); //Possible crash
        QString message = result; //Possible crash
