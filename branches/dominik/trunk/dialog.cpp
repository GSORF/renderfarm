    ...
    ui(new Ui::Dialog)
{
    ...
    ui->txtConsole->appendPlainText("Drag&Drop for profit!");
    //--------------------------------------------------------------------------------------------------------------------------
    //-----------------------------------------------------UI-SERVER-END--------------------------------------------------------
    //--------------------------------------------------------------------------------------------------------------------------
    ...
    connect(this, SIGNAL(setRemainingRenderTime(int)), &myWebServer, SLOT(onSetRemainingRenderTime(int)), Qt::QueuedConnection);
    connect(this, SIGNAL(setCurrentRenderTime(int)), &myWebServer, SLOT(onSetCurrentRenderTime(int)), Qt::QueuedConnection);
...
void Dialog::processPendingDatagrams()
{
    ...
    else if(tempString.indexOf("#") == 0)
    {
        ...
        qDebug() << "client: " << recordedHash << " - server: " << file_hash;
...
void Dialog::saveResultOnHarddisk(Result &result)
{
    ...
    int currentRenderTime = lastTaskTime;
    int remainingRenderTime = lastTaskTime * (framesTotal - framesRendered);
